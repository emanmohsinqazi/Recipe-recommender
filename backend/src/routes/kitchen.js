...
kitchenRouter.patch("/kitchen/items/:id/increment", userAuth, async (req, res) => {
  try {
    const item = await KitchenItem.findOne({ _id: req.params.id, user: req.user._id });
    ...
    await item.save({ validateBeforeSave: false }); // Skip schema validation
    ...
    res.status(400).json({ error: "Error incrementing quantity", details: error.message });
  }
});

kitchenRouter.patch("/kitchen/items/:id/decrement", userAuth, async (req, res) => {
  try {
    const item = await KitchenItem.findOne({ _id: req.params.id, user: req.user._id });
    ...
    res.status(400).json({ error: "Error decrementing quantity", details: error.message });
  }
});
